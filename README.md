...
### 双终端
- **基于 xterm.js + WebSocket + PTY** — 真实 Bash 终端，非模拟
...
### 服务管理
- **热重载** — 通过 Web UI 触发重新编译并重启服务，无需 SSH
...
|------|------|
| 后端 | Go 1.23, net/http |
| 终端 PTY | [creack/pty](https://github.com/creack/pty) v1.1.24 |
...
| 前端编辑器 | [CodeMirror 5.65](https://codemirror.net/5/) (Monokai) |
| 前端终端 | [xterm.js 5.3](https://xtermjs.org/) + xterm-addon-fit |
| Markdown | [marked 12.0](https://marked.js.org/) |
| UI | 纯 CSS，暗色极客风配色 |
| CDN | jsDelivr |
...
├── go.mod               # Go 模块定义
├── go.sum               # 依赖校验
├── fileserver           # 编译后的二进制文件
├── restart_fileserver.sh # 热重载脚本（等待旧进程退出 → 重新编译 → 启动）
